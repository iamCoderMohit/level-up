...
import { isTaskCompleted } from "@/lib/checkTaskComplete";
...
const levels: PathLevel[] = [
  {
    ...
    description:
      "Understanding sectioning elements, accessibility basics, and document flow.",
    status: "completed",
    stars: 3,
    best: "100%",
  },
  {
    ...
    description:
      "Mastering the cascade, class weighting, and advanced selectors.",
    status: "completed",
    stars: 2,
    best: "85%",
  },
  {
    ...
    description: "Building responsive 1-dimensional layouts and alignments.",
    status: "completed",
    stars: 3,
    best: "100%",
  },
  {
    ...
    description:
      "Creating complex 2-dimensional layouts with grid-template-areas.",
    status: "completed",
    stars: 2,
    best: "92%",
  },
  {
    level: 5,
    title: "JavaScript: The DOM API",
    description:
      "Learn how to select, modify, and animate HTML elements using vanilla JavaScript. This is crucial for interactivity.",
    status: "current",
    xpReward: 800,
    tags: ["querySelector", "EventListeners", "classList"],
  },
  {
    level: 6,
    title: "Async JavaScript & Fetch",
    description: "Understanding Promises, Async/Await, and API consumption.",
    status: "locked",
  },
  {
    level: 7,
...
function PathInfoCard() {
  ...
  useEffect(() => {
    api.get(`/path/getOneLevel/${slug}/${order}`).then((res) => {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 mb-8 lg:sticky lg:top-20">
      ...
      <h2 className="text-lg font-bold text-foreground mb-1">
        {level.title ? level.title : "loading"}
      </h2>
      <p className="text-sm text-muted-foreground mb-4">
        {level.description ? level.description : "loading"}
      </p>
      <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
        <span>Progress</span>
        <span className="text-primary font-semibold">Level {order} / 2</span>
      </div>
      <XPProgressBar current={progressOut} max={100} showLabel={false} size="sm" />
      <div className="grid grid-cols-2 gap-3 mt-4">
        <StatCard label="XP NEEDED" value={level.requiredXp} />
        <StatCard label="RANK" value="#402" />
      </div>
      <Link
        href={`/paths/${slug}/${order}/${level.order}`}
  ...
  return (
    <div className="rounded-xl bg-primary p-4 text-primary-foreground mb-6">
      ...
      <p className="text-sm mb-2">
        Create a pulsing button animation using only CSS keyframes.
      </p>
...
function LevelNode({ level, index }: { level: PathLevel; index: number }) {
  ...
  function getStarts() {
    if (level.difficulty === "EASY") return 2;
    if (level.difficulty === "MEDIUM") return 3;
    if (level.difficulty === "HARD") return 5;
  }
  const stars = getStarts();
  const user = useAuthStore((state) => state.user)
  const {order, slug} = useParams()
  ...
  return (
    <div className="relative flex items-start gap-4 lg:gap-0">
      ...
      <div className="flex flex-col items-center lg:absolute lg:left-1/2 lg:-translate-x-1/2 z-10">
        ...
        >
          {isTaskCompleted(user?.totalXp, order - 1, index) && (
        ...
        >
          ...
          {isTaskCompleted(user?.totalXp, order - 1, index) && (
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              <Link href={`/paths/${slug}/${order}/${level.order}`} className="text-primary hover:underline cursor-pointer">
                Replay Module
              </Link>
            </div>
          )}
                  ...
                  {level.tags.map((tag) => (
                    ...
                    >
                      {tag}
                        ...
                        {!isTaskCompleted(user?.totalXp, order - 1, index) && <Link
                href={`/paths/${slug}/${order}/${level.order}`}
      ...
      <div className="relative">
        ...
        <div className="flex flex-col gap-0 lg:gap-0">
          {level.tasks &&
            level.tasks.map((l, i) => (
              <div
                key={l.id}
                className="relative min-h-[120px] lg:min-h-[140px]"
              >
                <LevelNode level={l} index={i} />
              </div>
            ))}
        </div>
        ...
        <div className="mt-8 text-center">
          ...
          <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
            Milestone Project: Portfolio V1
          </p>
          <p className="text-xs text-muted-foreground">
            Combine everything {"you've"} learned to build your personal site.
          </p>
        </div>
      </div>
    </div>
